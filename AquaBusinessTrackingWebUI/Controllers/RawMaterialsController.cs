...
namespace AquaBusinessTrackingWebUI.Controllers
{
    public class RawMaterialsController : Controller
    {
        ...
        public RawMaterialsController(AuthorizedHttpClientService httpClientFactory, IOptions<ApiSettings> apiSettings, CurrentUserService currentUserService)
        {
        ...
        [HttpGet]
        public async Task<IActionResult> GetRawMaterialsList()
        {
            ...
            var response = await client.GetAsync($"{_apiSettings.BaseUrl}/RawMaterialIntake/details");
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            if (id.HasValue)
            {
                ...
                var client = _httpClientFactory.CreateClient();
                var response = await client.GetAsync($"{_apiSettings.BaseUrl}/RawMaterialIntake/getbyid/{id}");
                if (!response.IsSuccessStatusCode)
                    return RedirectToAction("GetRawMaterialsList");

                var json = await response.Content.ReadAsStringAsync();
                var dto = JsonConvert.DeserializeObject<RawMaterialIntakesDto>(json);

                var model = new ModalViewModel<RawMaterialIntakesDto>
                {
                    Entity = dto,
                    IsEdit = true,
            ...
            else
            {
                ...
                var model = new ModalViewModel<RawMaterialIntakesDto>
                {
                    Entity = new RawMaterialIntakesDto(),
                    IsEdit = false,
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(ModalViewModel<RawMaterialIntakesDto> model)
        {
            ...
            if (model.Entity.RecId != null && model.Entity.RecId > 0)
            {
                ...
                var result = await client.PutAsync($"{_apiSettings.BaseUrl}/RawMaterialIntake/update/{dto.RecId}", content);
            ...
            else
            {
                ...
                var result = await client.PostAsync($"{_apiSettings.BaseUrl}/RawMaterialIntake/add", content);
                if (!result.IsSuccessStatusCode)
                {
                    var errorMessage = await result.Content.ReadAsStringAsync();
                }
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            var client = _httpClientFactory.CreateClient();
            var response = await client.DeleteAsync($"{_apiSettings.BaseUrl}/RawMaterialIntake/delete/{id}");
            if (response.IsSuccessStatusCode)
            {
                return RedirectToAction("GetRawMaterialsList");
            }
