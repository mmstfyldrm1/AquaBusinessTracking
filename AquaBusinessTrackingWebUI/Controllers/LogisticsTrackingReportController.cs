...
namespace AquaBusinessTrackingWebUI.Controllers
{
    ...
    public class LogisticsTrackingReportController : Controller
    {
        ...
        public async Task<IActionResult> GetLogisticsTrackingReportList()
        {
            ...
            var values = JsonConvert.DeserializeObject<List<LogisticsTrackingReportDto>>(json);
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            if (id.HasValue)
            {
                ...
                var json = await response.Content.ReadAsStringAsync();
                var dto = JsonConvert.DeserializeObject<LogisticsTrackingReportDto>(json);

                var model = new ModalViewModel<LogisticsTrackingReportDto>
                {
                    Entity = dto,
                    IsEdit = true,
                    ModalTitle = "Lojistik Takip Raporu Güncelle",
                    FormAction = "Edit"
                };
            ...
            else
            {
                ...
                var model = new ModalViewModel<LogisticsTrackingReportDto>
                {
                    ...
                    IsEdit = false,
                    ModalTitle = "Lojistik Takip Raporu Ekle",
                    FormAction = "Edit"
                };
                return PartialView("_Edit", model);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(ModalViewModel<LogisticsTrackingReportDto> model)
        {
            ...
            if (model.Entity.RecId != null && model.Entity.RecId > 0)
            {
                ...
                var result = await client.PutAsync($"{_apiSettings.BaseUrl}/LogisticsTrackingReport/update/{dto.RecId}", content);
