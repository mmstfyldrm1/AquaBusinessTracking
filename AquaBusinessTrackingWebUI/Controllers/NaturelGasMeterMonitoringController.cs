...
namespace AquaBusinessTrackingWebUI.Controllers
{
    ...
    public class NaturelGasMeterMonitoringController : Controller
    {
        ...
        public async Task<IActionResult> GetNaturelGasMeterList()
        {
            ...
            var values = JsonConvert.DeserializeObject<List<NaturelGasMeterMonitoringDto>>(json);
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            if (id.HasValue)
            {
                ...
                var client = _httpClientFactory.CreateClient();
                var response = await client.GetAsync($"{_apiSettings.BaseUrl}/NaturelGasMeterMonitoring/getbyid/{id}");
                if (!response.IsSuccessStatusCode)
                    return RedirectToAction("GetNaturelGasMeterList");

                var json = await response.Content.ReadAsStringAsync();
                var dto = JsonConvert.DeserializeObject<NaturelGasMeterMonitoringDto>(json);

                var model = new ModalViewModel<NaturelGasMeterMonitoringDto>
                {
                    Entity = dto,
                    IsEdit = true,
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(ModalViewModel<NaturelGasMeterMonitoringDto> model)
        {
            ...
            if (model.Entity.RecId != null && model.Entity.RecId > 0)
            {
                ...
                await client.PutAsync($"{_apiSettings.BaseUrl}/NaturelGasMeterMonitoring/update/{dto.RecId}", content);
            }
            else
            {
                dto.InsertDate = DateTime.Now;
                await client.PostAsync($"{_apiSettings.BaseUrl}/NaturelGasMeterMonitoring/add", content);
            }
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            var client = _httpClientFactory.CreateClient();
            var response = await client.DeleteAsync($"{_apiSettings.BaseUrl}/NaturelGasMeterMonitoring/delete/{id}");
            if (response.IsSuccessStatusCode)
                return Redirect("~/NaturelGasMeterMonitoring/GetNaturelGasMeterList");
